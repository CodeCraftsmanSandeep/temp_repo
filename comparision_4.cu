...
/*
// the below kernel will work for 2^8 + 1 blocks and 2^8 threads per block
template <typename T>
__global__ void deviceReductionKernel(const T* __restrict__ arr, const int n, T* __restrict__ accum){
    T sum = 0;
    const unsigned int workPerBlock = ((n >> 16) << 8);
    if(blockIdx.x == (1 << 8)){
        // last block
        // 2^0 to 2^15
        for(int i = workPerBlock * (1 << 8) + threadIdx.x; i < n; i += blockDim.x) sum += arr[i];
    }else{
        // 2^16 to 2^30
        unsigned int start = threadIdx.x + workPerBlock * blockIdx.x;

        #pragma unroll
        for(unsigned int ele = (1 << 30); ele >= (1 << 16); ele >>= 1){
            if(n & ele){
                // Total ele elements
                // ele / (2^8) elements per block
                // ele / (2^8 * 2^8) elements per thread
                #pragma unroll
                for(unsigned int i = 0; i < (ele >> 16); i++){
                    sum += arr[start];
                    start += blockDim.x;
                 }
            }
...
__global__ void deviceReductionKernel(const T* __restrict__ arr, const int n, T* __restrict__ accum){
    ...
    if(blockIdx.x == (1 << numBlocksPower)){
        ...
        if(n & (1 << 30)){
            for(unsigned int i = 0; i < (1 << (30 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
        ...
        if(n & (1 << 29)){
            for(unsigned int i = 0; i < (1 << (29 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }
        ...
        if(n & (1 << 28)){
            for(unsigned int i = 0; i < (1 << (28 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        if(n & (1 << 27)){
            for(unsigned int i = 0; i < (1 << (27 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        if(n & (1 << 26)){
            for(unsigned int i = 0; i < (1 << (26 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += blockDim.x;
             }   
        }   
        ...
        if(n & (1 << 25)){
            for(unsigned int i = 0; i < (1 << (25 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        ...
        if(n & (1 << 24)){
            for(unsigned int i = 0; i < (1 << (24 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        ...
        if(n & (1 << 23)){
            for(unsigned int i = 0; i < (1 << (23 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        ...
        if(n & (1 << 22)){
            for(unsigned int i = 0; i < (1 << (22 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        if(n & (1 << 21)){
            for(unsigned int i = 0; i < (1 << (21 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        ...
        if(n & (1 << 20)){
            for(unsigned int i = 0; i < (1 << (20 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   
        ...
        if(n & (1 << 19)){
            for(unsigned int i = 0; i < (1 << (19 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }
        ...
        if(n & (1 << 18)){
            for(unsigned int i = 0; i < (1 << (18 - (numBlocksPower + blockSizePower))); i++){
                sum += arr[start];
                start += (1 << blockSizePower);
             }   
        }   

        if constexpr (numBlocksPower + blockSizePower <= 17){
            if(n & (1 << 17)){
                for(unsigned int i = 0; i < (1 << (17 - (numBlocksPower + blockSizePower))); i++){
                    sum += arr[start];
                    start += (1 << blockSizePower);
                }   
            }
        ...
        if constexpr (numBlocksPower + blockSizePower <= 16){
            if(n & (1 << 16)){
                for(unsigned int i = 0; i < (1 << (16 - (numBlocksPower + blockSizePower))); i++){
                    sum += arr[start];
                    start += (1 << blockSizePower);
                }
            }
